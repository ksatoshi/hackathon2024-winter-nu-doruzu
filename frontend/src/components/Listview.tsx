    ...
    <div className='flex max-h-screen overflow-y-hidden '>
      <div className="">
        {selectedCompanyId !== null && (
          <SelectedCompanyInfo selectedCompanyId={selectedCompanyId} selectedCompanyName={selectedCompanyName} onClose={() => {
    ...
    <div className="p-4 max-w-2xl w-full bg-zinc-800 max-h-screen h-full">
      ...
      {releases.length > 0 ? (
        releases.map((release: ReleaseInfoProps) => (
          <Link href={release.url} target="_blank" key={release.release_id} className="flex mb-4 gap-2 hover:bg-zinc-700 ">
            <img src={release.main_image} alt="main_image" className='max-w-40 max-h-40 min-h-28' />
